...
from cv_tfq.cv_subroutines import (
    ComputationalLayerBinary,
    ComputationalLayerInteger,
    discrete_continuous,
)
from cv_tfq.cv_subroutines import QFT, centeredQFT, kick_position, kick_momentum
from cv_tfq.cv_utils import domain_bin, plot_wfs, prep_state_binary, prep_state_integer
import unittest
...
class TestSubroutines(unittest.TestCase):
    def test_verify_centered_fourier(self):
        """
    ...
    def test_phi_fourier_consistency(self, quiet=True):
        """
        ...
        for n in [2, 5, 6]:
            ...
            for x_index in range(N):
                ...
                for k, (v2, v3) in enumerate(zip(phi2, phi_ana)):
                    if math.isclose(v2.real, 0, abs_tol=0.1) and math.isclose(
                        v3.real, 0, abs_tol=0.1
                    ):
                        continue
                    quiet_print(
                        "  %2i    " % k,
                    ...
                    self.assertAlmostEqual(v2, v3, delta=1e-2)

    def test_phi_eigenequation(self, quiet=True):
        """
        ...
        for precision in [2, 5]:
            ...
            for k in range(2**precision):
                ...
                for d, (k2, k3) in enumerate(zip(phi1, x_ket_ana)):
                    ...
                    self.assertAlmostEqual(k2, k3, delta=1e-2)

    def test_phi_clock(self, quiet=True):
        """
        ...
        for n in [2, 3]:
            ...
            for circuit_n in range(N):
                new_circuit = circuit_collect[circuit_n] + discrete_continuous(
                    kick_angle, [test_pi]
                )
                circuit_collect.append(new_circuit)
                sim_wfs.append(
                    cirq.Simulator().simulate(new_circuit).final_state_vector
                )
            ...
            for i_kick, (psi1, psi1_ana) in enumerate(zip(sim_wfs, expected_wfs)):
                ...
                for k, (v2, v3) in enumerate(zip(psi1, psi1_ana)):
                    # dense coding
                    if math.isclose(np.abs(v2), 0, abs_tol=0.1) and math.isclose(
                        v3, 0, abs_tol=0.1
                    ):
                        continue
                    quiet_print(
    ...
    def test_phi_adder(self, quiet=True):
        """
        ...
        for n in [2, 3, 4]:
            ...
            for xA_index in range(N):
                ...
                for xB_index in range(N):
                    ...
                    padded_k_f = (
                        "0" * (n - len(bin(phi_f_ind)[2:])) + bin(phi_f_ind)[2:]
                    )
                    ...
                    phiB_prep = phiA_prep + ComputationalLayerBinary(
                        k_padded, all_qubits
                    )

                    phi_pi_operator = phiB_prep + discrete_continuous(
                        -s, [phi_control, pi_target]
                    )

                    # observe the outcome at the target register
                    phiB_i = cirq.Simulator().simulate(phiB_prep).final_state_vector
                    phiB_f_sim = (
                        cirq.Simulator().simulate(phi_pi_operator).final_state_vector
                    )
                    ...
                    quiet_print("domain=[%3.2f, %3.2f)" % (a, b))
                    quiet_print(
                        "INITIAL STATE: |xA=%3.2f>|xB=%3.2f>  = |%s>|%s>"
                        % (x_A, x_B, j, k)
                    )
                    quiet_print(
                        "EXPECTED FINAL STATE: |%s>|%s>"
    ...
    def test_phi_adder_stepwise(self, quiet=True):
        """
        Exponential of discretized momentum should push a state around like a clock:
        exp(-i*sqrt(2*pi/N)*s*Pi) |j> = (-1)^s |j-s>
        """
        ...
        for xA_index in range(N):
            ...
            for xB_index in range(N):
                ...
                quiet_print("domain=[%3.2f, %3.2f)" % (a, b))
                quiet_print(
                    "INITIAL STATE: |xA=%3.2f>|xB=%3.2f>  = |%s>|%s>" % (x_A, x_B, j, k)
                )
                quiet_print(
                    "EXPECTED FINAL STATE: |%s>|%s>"
                    % (phi_f_ana[0:n], phi_f_ana[n : 2 * n])
